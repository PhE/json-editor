...
JSONEditor.AbstractEditor = Class.extend({
  ...
  setContainer: function(container) {
    ...
    if(this.schema.field_id) this.container.setAttribute('data-field_id', this.schema.field_id);
    if(this.schema.field_array) this.container.setAttribute('data-field_array', this.schema.field_array);
  },
...
JSONEditor.defaults.editors.string = JSONEditor.AbstractEditor.extend({
  ...
  build: function() {
    ...
    // PhE
    if(this.schema.operation) {
      this.input.setAttribute('data-operation', this.schema.operation);
    }

    // PhE
    if(this.schema.recopy) {
      this.input.setAttribute('data-recopy', this.schema.recopy);
    }
  ...
  addRow: function(value) {
    ...
    if(!this.hide_delete_buttons) {
      ...
      self.rows[i].delete_button.addEventListener('click',function(e) {
        ...
        self.setValue(newval);
        self.onChange(true);
        $(self.table).trigger('tablechange');
      });
      controls_holder.appendChild(self.rows[i].delete_button);
    }
...
JSONEditor.defaults.editors.multiselect = JSONEditor.AbstractEditor.extend({
  ...
  build: function() {
    ...
    if((!this.schema.format && this.option_keys.length < 8) || this.schema.format === "checkbox") {
      ...
      this.control = this.theme.getMultiCheckboxHolder(this.controls,this.label,this.description, this.tooltip);
    }
    else {
      this.input_type = 'select';
